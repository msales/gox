...
func TestContainsAtLeastOneUint(t *testing.T) {
	tests := []struct {
		name     string
		slice    []uint
		contains []uint
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := ContainsAtLeastOneUint(tt.slice, tt.contains...)
			if tt.want != got {
				t.Errorf("Got %+v, want %+v", got, tt.want)
			}
		})
...
func BenchmarkContainsAtLeastOneUint_Single(b *testing.B) {
	haystack := []uint{1, 2, 3, 4, 5, 6}
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		ContainsAtLeastOneUint(haystack, 3)
	}
}

func BenchmarkContainsAtLeastOneUint_Multiple(b *testing.B) {
	haystack := []uint{1, 2, 3, 4, 5, 6}
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		ContainsAtLeastOneUint(haystack, 2, 3)
	}
}
