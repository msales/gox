...
func TestContainsAtLeastOneInt64(t *testing.T) {
	tests := []struct {
		name     string
		slice    []int64
		contains []int64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := ContainsAtLeastOneInt64(tt.slice, tt.contains...)
			if tt.want != got {
				t.Errorf("Got %+v, want %+v", got, tt.want)
			}
		})
...
func BenchmarkContainsAtLeastOneInt64_Single(b *testing.B) {
	haystack := []int64{1, 2, 3, 4, 5, 6}
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		ContainsAtLeastOneInt64(haystack, 3)
	}
}

func BenchmarkContainsAtLeastOneInt64_Multiple(b *testing.B) {
	haystack := []int64{1, 2, 3, 4, 5, 6}
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		ContainsAtLeastOneInt64(haystack, 2, 3)
	}
}
